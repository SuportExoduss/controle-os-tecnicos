...
import { getAllReports, updateReport } from '../../services/database/reportService';
...
import {
  ...
  Info, Lock, LogIn, LogOut, Sun, Moon, ClipboardEdit,
} from 'lucide-react';
...
export const Dashboard = () => {
  ...
  const handleDashLogout = async () => {
    try { await logoutUser(); toast.success('Modo edição desativado'); } catch {}
  };
  ...
  const fetchReports = async () => {
    try {
      ...
      const arr = data.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      setReports(arr);
      filterReports(arr, '', '', '');
  ...
  const handleSearch = (from, to, tech) => {
    ...
    filterReports(reports, from, to, tech);
  };
  ...
  const scrollDays = (() => {
    ...
    while (cur <= endD) {
      ...
      const total = reports.filter(r => r.date === key).reduce((acc, r) => acc + (r.totalOrders || 0), 0);
      days.push({
        date: key,
        day: cur.getDate(),
  ...
  const handleExcelGenerate = () => {
    ...
    // filtra registros brutos (todos os dias com O.S no período) sem agrupar
    const records = reports
      .filter(r => r.date >= excelFrom && r.date <= excelTo && (r.totalOrders || 0) > 0)
  ...
  const metrics = [
    ...
    { icon: ClipboardList, label: 'Total O.S', value: summary.totalOrders, color: S.green, bg: '#0d2d1f', glow: 'rgba(52,211,153,0.12)' },
    ...
    <div style={{ minHeight: '100vh', width: '100%', display: 'flex', flexDirection: 'column', background: S.bg }}>
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 30, background: S.card, borderBottom: `1px solid ${S.border}` }}>
        <div className="r-maxw" style={{ padding: '0 16px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
            <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'linear-gradient(135deg, #3b82f6, #6366f1)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 16px rgba(99,102,241,0.35)', flexShrink: 0 }}>
            ...
            <div>
              <div style={{ color: S.text, fontWeight: 800, fontSize: '14px', lineHeight: 1 }}>IBIUNET</div>
              <div style={{ color: isLogged ? '#34d399' : S.muted, fontSize: '11px', marginTop: '2px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                {isLogged ? <><Check size={10}/>{profile.nickname}</> : 'Dashboard'}
              </div>
            </div>
            {/* Toggle tema */}
            ...
            {/* Botão ADM */}
            <button onClick={() => { if (isLogged) navigate('/admin'); else { setRedirectAfterLogin('/admin'); setShowLoginModal(true); } }} title="Ir para o painel Admin"
      ...
      <main style={{ flex: 1, width: '100%', display: 'flex', flexDirection: 'column', gap: '16px' }} className="r-page-pad r-maxw">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
          className="r-metrics">
          {metrics.map(({ icon: Icon, label, value, color, bg, glow, small }) => (
            <div key={label} style={{ background: S.card, border: `1px solid ${S.border}`, borderRadius: '16px', padding: '20px', position: 'relative', overflow: 'hidden' }}>
            ...
            <motion.div key={report.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.03 }}
              ...
              <div onClick={() => setExpandedId(expandedId === report.id ? null : report.id)}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                  <span className="r-badge-os" style={{ background: '#0f1d35', color: S.blue, fontSize: '12px', padding: '4px 10px', borderRadius: '999px', fontWeight: 700, border: '1px solid #1e3a5f', alignItems: 'center', gap: '4px' }}>
            ...
            <div style={{ width: '100%', maxWidth: '500px', background: S.surface, border: `1px solid ${S.border}`, borderRadius: '20px', padding: 'clamp(16px, 5vw, 28px)', boxShadow: '0 40px 100px rgba(0,0,0,0.8)', maxHeight: '90vh', overflowY: 'auto' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <div>
                    <div style={{ color: S.text, fontWeight: 800, fontSize: '16px' }}>Editar Registro</div>
                    <div style={{ color: S.muted, fontSize: '12px' }}>{editReport.technicianName}</div>
                  </div>
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: S.muted, letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '6px' }}>Total de O.S</label>
                  <input type="number" value={editReport.totalOrders} onChange={e => setEditReport(p => ({ ...p, totalOrders: e.target.value }))}
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, color: S.muted, letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '8px' }}>Tipos de Serviço</label>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                    {SERVICE_TYPES.map(svc => {
                      const active = (editReport.serviceTypes || []).includes(svc);
                      return (
                        <button key={svc} type="button"
                          onClick={() => {
                            const cur = editReport.serviceTypes || [];
                            setEditReport(p => ({ ...p, serviceTypes: active ? cur.filter(s => s !== svc) : [...cur, svc] }));
                          }}
                          style={{ padding: '5px 12px', borderRadius: '999px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', border: '1px solid', transition: 'all 0.15s', background: active ? '#1e3a5f' : 'transparent', color: active ? S.blue : S.muted, borderColor: active ? '#1e3a5f' : S.border }}>
                          {svc}
                        </button>
                      );
                    })}
                ...
                <div style={{ display: 'flex', gap: '12px', marginTop: '4px' }}>
                  ...
                  <button onClick={handleSaveEdit} disabled={editLoading}
                    ...
                    {editLoading ? <Spinner /> : <><Check size={16}/>Salvar</>}
                  </button>
                </div>
              </div>
            </div>
              ...
              <div style={{ width: '100%', maxWidth: '440px', background: S.surface, border: '1px solid #047857', borderRadius: '20px', boxShadow: '0 0 40px rgba(16,185,129,0.2)', padding: 'clamp(20px, 5vw, 28px)' }}>
                ...
                <div className="r-excel-dates" style={{ display: 'grid', gap: '12px', marginBottom: '20px' }}>
                  <div>
                    ...
                    <input type="date" value={excelFrom} onChange={e => setExcelFrom(e.target.value)} onClick={e => { try { e.target.showPicker(); } catch {} }}
                  ...
                  <div>
                    ...
                    <input type="date" value={excelTo} onChange={e => setExcelTo(e.target.value)} onClick={e => { try { e.target.showPicker(); } catch {} }}
                      style={{ width: '100%', padding: '11px 14px', borderRadius: '10px', background: S.input2, border: '1px solid #047857', color: S.text, fontSize: '14px', outline: 'none', boxSizing: 'border-box', colorScheme: mode, cursor: 'pointer' }} />
                  </div>
                </div>
