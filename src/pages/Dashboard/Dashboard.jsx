...
import {
  ...
  Info, LogIn, RotateCcw, Trash2, CalendarCheck,
} from 'lucide-react';
...
export const Dashboard = () => {
  ...
  // Período efetivamente exibido (para buscar ausências do card ao expandir).
  const [effFrom, setEffFrom] = useState('');
  const [effTo, setEffTo] = useState('');
  // Ausências (dias zerados c/ observação) por técnico — busca SÓ ao expandir o card.
  ...
  // do técnico no período exibido — 1 leitura por técnico, cacheada. Cards não
  // expandidos não geram leitura nenhuma (economia de cota).
  useEffect(() => {
    ...
    getReportsByTechnicianAll(name)
      .then(arr => {
        if (cancelled) return;
  ...
  return (
    <div style={{ minHeight: '100vh', width: '100%', display: 'flex', flexDirection: 'column', background: S.bg }}>
      ...
      <AreaTopbar
        ...
        onTheme={toggleTheme}
        onPrimary={() => { if (isLogged) navigate('/fibra/admin'); else { setRedirectAfterLogin('/fibra/admin'); setShowLoginModal(true); } }}
