...
import { saveDailyReport, getReportsByTechnician, deleteAllReportsByTechnician, deleteAllReportsByDate } from '../../services/database/reportService';
...
export const Admin = () => {
  ...
  const fetchStatus = async (refDate) => {
    ...
    try {
      ...
      // query simples sem orderBy (evita necessidade de índice composto)
      const snap = await getDocs(query(collection(db, 'daily_reports'), where('date', '==', refDate)));
  ...
  const handleLogout = async () => { try { await logoutUser(); navigate('/login'); } catch { toast.error('Erro ao sair'); } };
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      for (let i = 0; i < total; i++) {
        try {
          await saveDailyReport(importData.records[i]);
          saved++;
        } catch { skipped++; }
        setImportProgress(Math.round(((i + 1) / total) * 100));
      }
      toast.success(`Importação concluída! ${saved} registros salvos${skipped > 0 ? `, ${skipped} ignorados` : ''}.`);
      setShowImport(false);
      setImportData(null);
        ...
        <div className="r-maxw" style={{ height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ color: profile?.nickname ? S.green : S.muted, fontSize: '11px', marginTop: '2px', display: 'flex', alignItems: 'center', gap: '4px' }}>
            ...
            {/* Botão Dashboard */}
            <button onClick={() => navigate('/dashboard')} title="Ir para o Dashboard"
              ...
              <div style={{ padding: 'clamp(16px, 5vw, 28px)', display: 'flex', flexDirection: 'column', gap: '24px' }}>
                ...
                <div className="r-form-row">
                  <div>
                    ...
                    <DarkInput S={S} type="date" name="date" value={formData.date} onChange={handleChange} onClick={e => { try { e.target.showPicker(); } catch {} }} style={{ colorScheme: mode, cursor: 'pointer', borderColor: formData.date !== localDate() ? S.orange : undefined }} />
